...
(function () {
  ...
  function basePath() {
    const script = document.currentScript || Array.from(document.scripts).find((item) => item.src && (item.src.includes('/frontend/theme-pulse/assets/js/app.js') || item.src.includes('/frontend/theme-default/assets/js/app.js')));
    if (!script || !script.src) return '';
    try {
      const path = new URL(script.src, window.location.origin).pathname;
      const markers = ['/frontend/theme-pulse/assets/js/app.js', '/frontend/theme-default/assets/js/app.js'];
