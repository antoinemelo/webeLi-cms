...
function drop(index:number){ if(draggedIndex.value===null||draggedIndex.value===index)return;const item=form.sections.splice(draggedIndex.value,1)[0];form.sections.splice(index,0,item);draggedIndex.value=null;dirty.value=true; }
...
function restoreDefaults(){
  ...
  form.analytics={dedupe_minutes:30,retention_days:90}; dirty.value=true;
}
  ...
  <section class="shop-system-editor">
    ...
    <form v-if="!loading&&source" class="shop-system-grid" @input="markDirty" @change="markDirty" @submit.prevent="save">
      ...
      <section class="card shop-system-card shop-system-sections">
        ...
        <article v-for="(section,index) in form.sections" :key="section.key" class="shop-section-row" draggable="true" @dragstart="draggedIndex=index" @dragover.prevent @drop.prevent="drop(index)">
          <div class="shop-section-row__top"><span class="drag-handle" :title="ui.drag" aria-hidden="true">⋮⋮</span><label><input v-model="section.enabled" type="checkbox"> <strong>{{ section.key }}</strong></label><div><button class="btn btn-sm" type="button" :disabled="index===0" @click="move(index,-1)" :aria-label="`${t('shopSystem.moveUp')} ${section.key}`">↑</button><button class="btn btn-sm" type="button" :disabled="index===form.sections.length-1" @click="move(index,1)" :aria-label="`${t('shopSystem.moveDown')} ${section.key}`">↓</button></div></div>
