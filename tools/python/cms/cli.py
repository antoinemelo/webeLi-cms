...
from tools.python.commands import audit, backup, business, docs, e2e, export, init, instance, inventory, migrate, outbox, qualify, release, smoke, test, validate
from tools.python.lib.release_metadata import load_release_metadata

COMMANDS = {'init': init.run, 'rebuild': init.rebuild, 'validate': validate.run, 'qualify': qualify.run, 'audit': audit.run, 'test': test.run, 'e2e': e2e.run, 'export': export.run, 'backup': backup.run, 'migrate': migrate.run, 'instance': instance.run, 'release': release.run, 'smoke': smoke.run, 'docs': docs.run, 'business': business.run, 'inventory': inventory.run, 'outbox': outbox.run}
...
def parser() -> argparse.ArgumentParser:
    ...
    inventory.configure(sub.add_parser('inventory', help='Diagnostiquer et reconstruire le stock Sale/Business.'))
    outbox.configure(sub.add_parser('outbox', help='Inspecter et piloter l’outbox transactionnelle.'))
    return p
