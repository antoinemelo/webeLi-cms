...
def run(ctx,args):
    ...
    chain_flags = args.ci or args.build_admin or args.run_essential_validators or args.package or args.deploy
    if args.verify_archive and not chain_flags:
        a = []
        if args.archive:
            a += ['--archive', args.archive]
        if ctx.json_output:
            a.append('--json')
        return python_script(ctx,'tools/python/operations/deployment/d4_verify_release_archive.py',a,timeout=_release_timeout(ctx))
    if args.archive:
        raise ValueError("--archive s'utilise avec release --verify-archive sans --package, --ci, --build-admin ni --deploy.")
