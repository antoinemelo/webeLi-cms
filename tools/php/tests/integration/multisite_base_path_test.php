 ...
 * @return void
 */
function runBasePathScenario(TestHarness $h, string $appBasePath): void
{
    ...
    try {
        ...
        $pdo->exec("INSERT INTO site_domains(id,site_id,host,base_path,scheme,is_primary,is_active,enforce_https) VALUES(1,1,'webe.li','','https',1,1,1),(10,10,'webe.li','/site-a','https',1,1,1),(11,11,'webe.li','" . $appBasePath . "/site-b','https',1,1,1)");

        $repo = new SiteRepository($db, ['cms' => ['default_site_key' => 'main']]);

        $siteA = $repo->resolveCurrentSite('webe.li', '/site-a/', true);
        $h->assertSame(10, (int) $siteA['id'], $appBasePath . ' site A resolved from request-visible subsite path');
        $h->assertSame('/site-a', $siteA['matched_request_base_path'], $appBasePath . ' site A request base strips APP_BASE_PATH');
        $h->assertSame($appBasePath . '/site-a', $siteA['matched_base_path'], $appBasePath . ' site A public base includes APP_BASE_PATH');
        $h->assertSame('https://webe.li' . $appBasePath . '/site-a', $siteA['current_base_url'], $appBasePath . ' site A public URL includes APP_BASE_PATH');

        $siteB = $repo->resolveCurrentSite('webe.li', '/site-b/', true);
        $h->assertSame(11, (int) $siteB['id'], $appBasePath . ' site B resolved when DB already stores the full public path');
        $h->assertSame('/site-b', $siteB['matched_request_base_path'], $appBasePath . ' site B request base strips stored APP_BASE_PATH');
        $h->assertSame($appBasePath . '/site-b', $siteB['matched_base_path'], $appBasePath . ' site B public base is not double-prefixed');
        $h->assertSame('https://webe.li' . $appBasePath . '/site-b', $siteB['current_base_url'], $appBasePath . ' site B URL is not double-prefixed');

        $main = $repo->resolveCurrentSite('webe.li', '/', true);
        $h->assertSame(1, (int) $main['id'], $appBasePath . ' main site resolved at app root');
        $h->assertSame('', $main['matched_request_base_path'], $appBasePath . ' main request base is empty below APP_BASE_PATH');
        ...
        $h->assertSame('/site-a', $scoped->server['CMS_SITE_BASE_PATH'], $appBasePath . ' server keeps request-visible site base');
        $h->assertSame($appBasePath . '/site-a', $scoped->server['CMS_SITE_DOMAIN_BASE_PATH'], $appBasePath . ' server exposes public domain base separately');
    } finally {
