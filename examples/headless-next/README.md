# Exemple headless Next.js App Router

Exemple volontairement minimal pour consommer l’API publique headless v1 d’DEC CMS depuis Next.js. Il ne s’agit pas d’un thème complet : le but est de montrer une intégration claire, vendable à un développeur front-end tiers.
...
Le SDK local est référencé par `file:../../packages/amcms-client`. S’il n’est pas encore compilé ou utilisable, `lib/amcms.ts` bascule automatiquement sur `fetch` natif pour les mêmes endpoints.
...
- `app/page.tsx` : page d’accueil exemple avec route, menu, articles, contenu par slug et recherche.
- `app/[...path]/page.tsx` : page dynamique basée sur le path public résolu par `GET /api/v1/route`.
- `app/[...path]/page.tsx` : exemple de `generateMetadata()` SEO à partir de la réponse API.
- `lib/amcms.ts` : usage du SDK TypeScript si disponible, fallback `fetch` natif sinon, gestion propre des erreurs API.
- `lib/payload.ts` : petites fonctions de lecture tolérantes des payloads normalisés.
