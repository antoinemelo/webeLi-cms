...
final class SaleFulfillmentService
{
    ...
    public function createOperation(int $siteId, int $orderId, array $payload, ?int $actorId): array
    {
        ...
        if(!in_array((string)$order['payment_status'],['authorized','paid'],true)) throw new SaleValidationException('sale.fulfillment.payment_required');
    ...
    public function transitionOperation(int $siteId, int $fulfillmentId, string $status, array $payload, ?int $actorId): array
    {
        ...
        if(in_array($status,['shipped','handed_over','delivered'],true)){
            $order=$this->db()->one('SELECT payment_status FROM sale_orders WHERE id=? AND site_id=?',[(int)$operation['order_id'],$siteId]);
            if($order===null || (string)$order['payment_status']!=='paid') throw new SaleValidationException('sale.fulfillment.payment_required_before_delivery');
        }
