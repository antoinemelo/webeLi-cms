...
";
    ...
    private function renderLogin(string $error, string $notice, string $email = '', string $challenge = 'email'): string
    {
        ...
        if ($isEmailStep) {
            ...
            $credentialFields = '<input type="hidden" name="challenge" value="totp"><input type="hidden" name="email" value="' . $emailValue . '"><p class="field"><label for="password">Mot de passe</label><span class="password-field"><input id="password" name="password" type="password" autocomplete="current-password" required autofocus><button class="password-toggle" type="button" data-password-toggle="password" aria-label="Afficher le mot de passe" aria-pressed="false"><svg class="password-toggle__icon" viewBox="0 0 16 16" aria-hidden="true" focusable="false"><path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8ZM1.173 8A13.133 13.133 0 0 1 8 3.5 13.133 13.133 0 0 1 14.827 8 13.133 13.133 0 0 1 8 12.5 13.133 13.133 0 0 1 1.173 8Z"/><path d="M8 5.5A2.5 2.5 0 1 1 8 10.5 2.5 2.5 0 0 1 8 5.5ZM8 6.5A1.5 1.5 0 1 0 8 9.5 1.5 1.5 0 0 0 8 6.5Z"/></svg></button></span></p><p class="field"><label for="totp_code">Code d’application TOTP</label><input id="totp_code" name="totp_code" type="text" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" minlength="6" maxlength="6" placeholder="123456" required></p>';
        } else {
