...
1. Décompressez l’archive dans un chemin définitif. Les commandes citées utilisent des guillemets afin de supporter les espaces : `cd "/chemin/avec espaces/mod"`.
2. Copiez `ops/.env.example` vers la configuration locale utilisée par le déploiement et adaptez `APP_BASE_PATH`, l’URL publique, les secrets et le mode production.
