---
title: Guide utilisateur
audience:
  - editor
  - publisher
  - seo
status: stable
version: 1.0
last_verified: 2026-06-14
source_of_truth: manual
owners:
  - core
document_type: guide
generated: false
---
# Guide utilisateur

Le guide suit les parcours du back-office. Les menus visibles dépendent du rôle, des permissions et du site actif ; masquer une action dans l’interface ne remplace jamais l’autorisation côté serveur.

## Contenus

- [Créer, modifier et prévisualiser](content/create-edit.md)
- [Champs, fieldsets et blocs](content/fields-and-blocks.md)
- [Statuts, révisions et recherche éditoriale](content/statuses-revisions-search.md)

## Publication

- [Réviser et publier](publication/review-publish.md)
- [Dépublier, archiver, supprimer ou restaurer](publication/unpublish-archive-delete-restore.md)

## Médias, navigation et recherche

- [Bibliothèque de médias](media/library.md)
- [Variantes, usages et suppression sûre](media/variants-usage-safe-deletion.md)
- [Menus](navigation-taxonomies/menus.md) et [taxonomies](navigation-taxonomies/taxonomies.md)
- [Recherche](search/use-search.md)

## SEO, formulaires et services

- [Workflow SEO](seo/seo-workflow.md)
- [URL, canonical, hreflang, robots et données structurées](seo/urls-canonical-robots-structured-data.md)
- [Audit, redirections et dépannage SEO](seo/audit-redirects-troubleshooting.md)
- [Formulaires, recherche, imports, exports et IA](forms-cookies/forms-search-import-export-ai.md)
- [Consentement cookies](forms-cookies/cookie-consent.md)
- [Assistant IA](ai-assistant/use.md)

## Compte et dépannage

- [Connexion, profil et contexte](getting-started/sign-in-profile-context.md)
- [Sécurité du compte et 2FA](getting-started/account-security-2fa.md)
- [Dépannage utilisateur](troubleshooting.md)
